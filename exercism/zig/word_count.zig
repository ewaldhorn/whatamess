...
pub fn countWords(allocator: mem.Allocator, s: []const u8) !std.StringHashMap(u32) {
    ...
    while (splitter.next()) |tmp_s| {
        ...
        const value = result.get(tmp_s[word_start..word_end]);
        if (value) |v| {
            try result.put(tmp_s[word_start..word_end], v + 1);
        } else {
            ...
            const tmp = std.fmt.allocPrint(allocator, "{s}", .{tmp_s[word_start..word_end]}) catch "ERROR";
