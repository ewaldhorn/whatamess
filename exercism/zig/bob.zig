...
pub fn response(s: []const u8) []const u8 {
    ...
    for (s) |c| {
        isEmpty = isEmpty and std.ascii.isWhitespace(c);

        if (!hasLetter) {
            hasLetter = std.ascii.isAlphabetic(c);
        }

        if (c == '?') {
            isQuestion = true;
        } else if (isQuestion and !std.ascii.isWhitespace(c)) {
            isQuestion = false;
        }

        if (c != std.ascii.toUpper(c)) {
            isYelling = false;
        }
    }

    isYelling = isYelling and hasLetter;

    if (s.len == 0 or isEmpty) {
        return "Fine. Be that way!";
    } else if (isYelling and isQuestion) {
        return "Calm down, I know what I'm doing!";
    } else if (isQuestion) {
        return "Sure.";
    } else if (isYelling and hasLetter) {
