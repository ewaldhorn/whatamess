...
func emailEntryFromRow(row *sql.Rows) (*EmailEntry, error) {
	...
	tmpTime := time.Unix(confirmedAt, 0)

	return &EmailEntry{Id: id, Email: email, ConfirmedAt: &tmpTime, OptOut: optOut}, nil
}
...
func UpdateEmail(db *sql.DB, emailEntry EmailEntry) error {
	tmpTime := emailEntry.ConfirmedAt.Unix()
	...
	_, err := db.Exec(`
		...
		ON CONFLICT(email) DO UPDATE SET
			confirmed_at=?,
			opt_out=?`,
		emailEntry.Email, tmpTime, emailEntry.OptOut, tmpTime, emailEntry.OptOut)
...
func GetEmailBatch(db *sql.DB, parameters GetEmailBatchQueryParameters) ([]EmailEntry, error) {
	...
	rows, err := db.Query(`
		...
		ORDER BY id ASC
		LIMIT ? OFFSET ?`,
		parameters.Count, (parameters.Page-1)*parameters.Count)
	...
	emails := make([]EmailEntry, 0, parameters.Count)
