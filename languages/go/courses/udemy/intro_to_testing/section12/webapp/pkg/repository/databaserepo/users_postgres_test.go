...
func TestPostgresDBRepo_InsertUserImage(t *testing.T) {
	...
	newID, err := testRepo.InsertUserImage(imageData)
	if err != nil {
		t.Errorf("unexpected error adding a new image: %s", err)
	}

	if newID != 1 {
		t.Errorf("expected new ID to be 1, got %d", newID)
	}

	imageData.UserID = 11
	_, err = testRepo.InsertUserImage(imageData)
	if err == nil {
		t.Error("expected an error inserting an image for an unknown user")
	}
}
